...
AFRAME.registerComponent('sponsor-wall-placement', {
  init() {
    this._placed = false;
    this._place = () => {
      ...
      const walls = window.MUSEO_WALL_MESHES || [];
      const obstacles = window.MUSEO_OBSTACLES || [];
      const roomLength = Math.max(0.1, bounds.maxZ - bounds.minZ);
      const centerZ = (bounds.minZ + bounds.maxZ) / 2;
      const y = spawn.y + 1.55;
      const probeX = bounds.maxX - 0.15;
      const ray = new THREE.Raycaster();
      const dir = new THREE.Vector3(1, 0, 0);

      const rawCandidates = [
        centerZ,
        bounds.minZ + roomLength * 0.25,
        bounds.minZ + roomLength * 0.75,
        bounds.minZ + Math.min(1.35, roomLength * 0.22),
        bounds.maxZ - Math.min(1.35, roomLength * 0.22)
      ];
      const candidates = rawCandidates.filter((z, i, arr) =>
        z > bounds.minZ + 0.5 &&
        z < bounds.maxZ - 0.5 &&
        arr.findIndex((v) => Math.abs(v - z) < 0.05) === i
      );

      let best = null;
      candidates.forEach((z) => {
        let wallX = bounds.maxX + 0.35;
        if (walls.length) {
          ray.set(new THREE.Vector3(probeX, y, z), dir);
          ray.far = 1.3;
          const hits = ray.intersectObjects(walls, true);
          if (!hits.length) return;
          wallX = hits[0].point.x - 0.018;
        }

        let nearest = Infinity;
        obstacles.forEach((o) => {
          if (!o.center) return;
          nearest = Math.min(nearest, Math.hypot(wallX - o.center.x, z - o.center.z));
        });
        if (!Number.isFinite(nearest)) nearest = 10;

        if (!best || nearest > best.score) best = { x: wallX, z, score: nearest };
      });

      if (!best) best = { x: bounds.maxX + 0.35, z: centerZ, score: 0 };

      this.el.object3D.position.set(best.x, y, best.z);
    ...
    window.setTimeout(this._place, 900);
  },
  remove() {
    const modelo = document.querySelector('#modelo');
