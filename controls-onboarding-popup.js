/* Persistent 10-second controls tutorial shown after the museum loader disappears. */
(() => {
  ...
  function installStyles() {
    ...
    style.textContent = `
      ...
      #controls-popup .controls-popup-card {
        ...
        box-sizing: border-box;
        border-radius: 16px;
        border: 1px solid rgba(74,63,51,.16);
        background: rgba(250,246,240,.98);
        box-shadow: 0 24px 70px rgba(20,12,24,.30);
        color: #342c25;
      }
      ...
      .controls-demo-wrap { width: 116px; height: 112px; flex: 0 0 116px; position: relative; }
      .controls-demo-keys {
        position: absolute;
        left: 9px;
        top: 0;
      ...
      .controls-demo-key {
        ...
        font-size: .68rem;
        font-weight: 800;
        box-shadow: 0 2px 0 rgba(74,63,51,.12);
      ...
      .controls-mouse-demo { width: 116px; height: 104px; flex: 0 0 116px; position: relative; display: grid; place-items: center; }
      ...
      .controls-drag-arrows { position: absolute; left: 0; right: 0; bottom: 4px; text-align: center; color: #7d3fa8; font-size: 1.2rem; letter-spacing: .12em; }
      ...
      @media (prefers-reduced-motion: reduce) {
        #controls-popup, #controls-popup .controls-popup-timer span, .controls-demo-key, .controls-mouse-shape, .controls-mouse-shape::after, .controls-mobile-joystick::after, .controls-mobile-swipe { animation: none !important; transition: none !important; }
      }
    `;
  ...
  function waitForMuseum() {
    ...
    const ready = () => loading.classList.contains('hidden');
    if (ready()) { window.setTimeout(showPopup, 300); return; }
    ...
    // Poll as a second path so the tutorial still appears if another script replaces classes.
    const poll = window.setInterval(() => {
      if (shown) { window.clearInterval(poll); observer.disconnect(); return; }
      if (ready() || getComputedStyle(loading).opacity === '0') {
